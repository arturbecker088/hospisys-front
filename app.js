...
function finalizarTriagemComSucesso(classificacaoFinal) {
    const nome = document.getElementById('nome').value.trim();
    ...
    const pesos = { "EMERGÊNCIA": 1, "MUITO URGENTE": 2, "URGENTE": 3, "POUCO URGENTE": 4, "NÃO URGENTE": 5 };

    filaPacientesSimulada.push({
        nome: nome,
        ...
        hora: horaAtual,
        riscoTexto: classificacaoFinal,
        riscoCor: coresRisco[classificacaoFinal].cor,
        weight: pesos[classificacaoFinal]
    });

    filaPacientesSimulada.sort((a, b) => a.weight - b.weight);
    atualizarTabelaFila();
...
function atualizarTabelaFila() {
    ...
    filaPacientesSimulada.forEach((p, idx) => {
        ...
        const sintomasHtml = p.sintomas && p.sintomas.length
            ? `<div class="tags-container">${p.sintomas.map(s => `<span class="tag-sintoma">${s}</span>`).join('')}</div>`
            : '';

        // Comorbidades
        const comorbHtml = p.comorbidades && p.comorbidades.length
            ? `<div class="tags-container">${p.comorbidades.map(c => `<span class="tag-comorbidade">${c}</span>`).join('')}</div>`
            : '';

        // Explicações clínicas (ocultas por padrão)
        const justTexto = p.justificativas && p.justificativas.length && p.justificativas[0] !== "Nenhum critério de risco específico assinalado nos questionários do protocolo clínico."
            ? p.justificativas.join('<br><br>')
            : null;

        const isEmergenciaDireta = p.protocolo === 'EMERGÊNCIA DIRETA';
        const explicacaoHtml = justTexto ? (
            isEmergenciaDireta
                ? `<div class="bloco-explicacao" style="margin-top:0.5rem;">
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td>
                    <div style="font-size:0.85rem; font-weight:600; color:var(--texto-mutado); margin-bottom:0.4rem;">📋 ${p.protocolo}</div>
                    ${sintomasHtml}
                    ${comorbHtml ? `<div style="margin-top:0.4rem;">${comorbHtml}</div>` : ''}
                    ${explicacaoHtml}
                </td>
                <td>
...
function chamarPaciente(index) {
    alert(`Chamando paciente: ${filaPacientesSimulada[index].nome}`);
...
function confirmarEmergenciaDireta() {
...
// Executa APENAS quando a página inteira e todos os scripts carregarem 100%
window.onload = () => {
